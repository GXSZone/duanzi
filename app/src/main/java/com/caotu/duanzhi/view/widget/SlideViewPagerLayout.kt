...
/**
 * 自定义的代替原生viewpager的懒加载问题,同时解决性能问题
 */
class SlideViewPagerLayout : FrameLayout, NestedScrollingChild2 {
...
//                startNestedScroll(ViewCompat.SCROLL_AXIS_VERTICAL)
    ...
    private inner class ViewHolderDelegate<ViewHolder : SlideViewHolder>(
        ...
        private fun ViewHolder?.prepare(direction: SlideDirection): ViewHolder {
            val holder = this
                    ?: adapter.onCreateViewHolder(context, this@SlideViewPagerLayout, mInflater)
        ...
        fun onDismissBackup(direction: SlideDirection) {
            backupViewHolder?.let { adapter.onViewDismiss(it, this@SlideViewPagerLayout, direction) }
        }

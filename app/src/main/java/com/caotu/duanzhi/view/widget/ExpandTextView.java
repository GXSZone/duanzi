...
/**
 * 可展开收起的的textview
 * https://github.com/lcodecorex/ExpandTextView
...
//        mAnimAlphaStart = typedArray.getFloat(R.styleable.ExpandTextView_animAlphaStart, DEFAULT_ANIM_ALPHA_START);
        ...
        arrowDrawablePadding = (int) typedArray.getDimension(R.styleable.ExpandTextView_arrowPadding, DevicesUtils.dp2px(2));
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                Rect bounds = mExpandDrawable.getBounds();
                if (event.getX() >= bounds.left && event.getX() <= bounds.right
                        && event.getY() >= bounds.top && event.getY() <= bounds.bottom) {
                    onClick();
                    return true;
                }
                break;
